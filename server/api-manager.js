...
class MindbodyQueries {
  constructor() {
    ...
    //it's a hash map, an array where the indexes are generated by the function
    //mindbodyQueries.getCacheHashCode(siteId + url). The value of each entry is a tuple of:
  ...
  getAuth() {
    ...
    this.requestNum++;
    //note that getAuth intentionally bypasses caching because it's a special GET request
    if (!this.atLimit()) {
  ...
  *//*eslint-enable */
  ...
  decorateAndMake(request) {
    ...
    let hashCode = undefined;
    if (request.verb === "GET") {
      hashCode = this.getCacheHashCode(request.siteId.toString() + request.url);
      let cacheTuple = this.cachedResults[hashCode];
      //console.log("MindbodyAccess cache request for " + request.url + " (" + hashCode + ")");// held value: " + JSON.stringify(cacheTuple));
      if (cacheTuple && cacheTuple.siteId === request.siteId && cacheTuple.url === request.url) {
        const msPerSecond = 1000;
        if (Date.now() - cacheTuple.creationTime < this.cacheMaxAgeInSeconds * msPerSecond) {
          //console.log("MindbodyAccess cache HIT! :)");
          return Promise.resolve(cacheTuple.result);
        } else {
          //console.log("MindbodyAccess cache removed expired entry");
          delete this.cachedResults[hashCode];
        }
      }
      //console.log("MindbodyAccess cache MISS! :(");
    }
      ...
      return request.makeRequest()
        .then((value) => {
          ...
          if (!value.PaginationResponse) {
            if (request.verb === "GET") {
              this.cachedResults[hashCode] = {
                siteId: request.siteId,
                url: request.url,
                creationTime: Date.now(),
                result: value,
              };
              //console.log("MindbodyAccess stored this cacheTuple " + JSON.stringify(this.cachedResults[hashCode]) + " at hash value: " + hashCode);
            }
          ...
          return Promise.all(allPagePromises)
            .then((responses) => {
              ...
              if (request.verb === "GET") {
                ...
                //console.log("MindbodyAccess stored this cacheTuple " + JSON.stringify(this.cachedResults[hashCode]) + " at hash value: " + hashCode);
                //console.log("MindbodyAccess stored multipage cacheTuple for " + initialUrl);
              }
  ...
  loadConfig(filename) {
    fs.readFile(filename, "utf8", (err, jsonString) => {
      if (err) {
        console.log("loadConfig: File read failed:", err);
        return;
      }
      try {
        ...
        if (filename === "./server/config/secrets.json") {
          ...
          this.cacheMaxAgeInSeconds = data.cacheMaxAgeInSeconds;
        }
        //^^ a shorter equivalent of this if+then is: Object.assign(this, JSON.parse(jsonString));
      } catch (parseErr) {
        console.log("loadConfig: Error parsing JSON string:", parseErr);
      }
    });
  }
